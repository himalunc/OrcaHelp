...
public class MJDMonitor extends PApplet {
...
public void initializeAdcMap(String server, String databaseName)
{
  try {
...
public Boolean exclude(String adcName)
{
...
public void decodeAdcValues(String database, JSONObject doc)
{
  try {
      ...
      if (!exclude(key)) {
        ...
        if (fValue!=0) {
          if (abs(fValue)<.001f) {
            ...
            if (fraction>0) {
              if (fraction>.001f) value = String.format("%.3f", Float.parseFloat(value));
...
public Boolean loadMostRecentAdcValues(String server, String database, String process)
{
  try {
    TreeMap dataBaseMap = (TreeMap)worldMap.get(database);
...
public void drawAll()
{
  ...
  if((String)timeStamp.get(databases[dbIndexToDraw]) == "Accessing")fill(0xffaaaaaa);
  else fill(0);
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#FFFFFF", "MJDMonitor" });
  }
}
